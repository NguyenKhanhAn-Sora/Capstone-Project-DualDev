...
class ChannelConversationDetailsSheet extends StatelessWidget {
  const ChannelConversationDetailsSheet._({
  ...
  static Future<void> show(
    ...
    return showModalBottomSheet<void>(
      ...
      useSafeArea: true,
      builder: (_) => ChannelConversationDetailsSheet._(
        channelId: channelId,
  ...
  Widget build(BuildContext context) {
    ...
    return MessagesChromeBuilder(
      builder: (context, chrome) => DraggableScrollableSheet(
        ...
        builder: (ctx, scrollController) => _ChannelSheetBody(
          chrome: chrome,
...
class _ChannelSheetBody extends StatefulWidget {
  const _ChannelSheetBody({
  ...
  @override
  State<_ChannelSheetBody> createState() => _ChannelSheetBodyState();
}

class _ChannelSheetBodyState extends State<_ChannelSheetBody>
  ...
  List<ChannelMessage> _searchResults = [];
  bool _searching = false;
  Timer? _debounce;

  // ── pinned ───────────────────────────────────────────────────────────
  bool _pinnedLoading = false;
  List<ChannelMessage> _pinnedMessages = [];
  ...
  Future<void> _loadPinned() async {
    ...
    try {
      final msgs = await ChannelMessagesService.getPinnedMessages(
        widget.channelId,
      );
  ...
  String get _displayChannelName {
    ...
    final translated = translateChannelName(widget.channelName, lang);
    return '#$translated';
  }

  Widget _buildHeader() {
    final initials = widget.channelName.isNotEmpty
        ? widget.channelName.trim()[0].toUpperCase()
        : '#';
    return Padding(
                ...
                children: [
                  ...
                  Text(
                    message.content.isEmpty
                        ? MessagesI18n.t('chat.pinnedScreen.emptyMessage')
                        : message.content,
                    maxLines: 2,
                    overflow: TextOverflow.ellipsis,
                    style: TextStyle(color: textColor, fontSize: 13),
