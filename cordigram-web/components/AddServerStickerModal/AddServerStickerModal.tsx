...
import styles from "@/components/AddServerEmojiModal/AddServerEmojiModal.module.css";
...
export default function AddServerStickerModal({
  isOpen,
  file,
  token,
  defaultServerId,
  ...
  const [targets, setTargets] = useState<serversApi.StickerUploadTarget[]>([]);
  ...
  useEffect(() => {
    ...
    trackBlobUrl(u);
    setImageSrc(u);
    setGif(isAnimatedSticker(file));
    setStickerName(nameFromFile(file));
    setCrop({ x: 0, y: 0 });
    setZoom(1);
    setCroppedAreaPixels(null);
    setFormErr(null);
    ...
    void serversApi
      ...
      .then((res) => {
        ...
        setSelectedServerId((cur) => {
          ...
          const preferred = res.targets.find(
            (t) => t.serverId === defaultServerId,
          );
  ...
  const canSubmit =
    ...
    !gifTooBig &&
    (gif ? !!imageSrc : !!croppedAreaPixels && !!imageSrc);

  const handleDone = async () => {
    if (!file || !selected || !canSubmit) return;
    const name = sanitizeStickerName(stickerName);
    if (!name) {
      setFormErr("Nhập tên sticker (chữ, số, gạch dưới).");
      return;
    }
    ...
    try {
      if (gif) {
        ...
        if (!imageSrc || !croppedAreaPixels) {
          setFormErr("Chọn vùng cắt cho sticker.");
          setSubmitting(false);
          return;
        }
        let blob = await exportSquareCropPng(imageSrc, croppedAreaPixels);
        blob = await compressImageBlobUnder(blob, STICKER_MAX_BYTES, "image/png");
        if (blob.size > STICKER_MAX_BYTES) {
          blob = await compressImageBlobUnder(blob, STICKER_MAX_BYTES, "image/webp");
        }
        if (blob.size > STICKER_MAX_BYTES) {
          setFormErr(
            `Sticker quá lớn sau khi nén. Tối đa ${Math.round(STICKER_MAX_BYTES / 1024)} KB.`,
          );
          setSubmitting(false);
          return;
        }
    ...
    >
      <div className={styles.modal} onMouseDown={(e) => e.stopPropagation()}>
        <header className={styles.header}>
          ...
          <h2 id="add-sticker-title" className={styles.title}>
            Thêm sticker
          </h2>
          <button
            type="button"
        ...
        <div className={styles.body}>
          ...
          <aside className={styles.side}>
            ...
            <div>
              <label className={styles.fieldLabel} htmlFor="sticker-name-input">
                Tên sticker <span className={styles.required}>*</span>
              </label>
            ...
            {selected && selected.remaining <= 0 ? (
              <p className={styles.err}>
                Máy chủ này đã hết chỗ sticker ({selected.max} tối đa).
              </p>
            ) : null}
