                ...
                <div className={styles.section}>
                  ...
                  {voiceChannels.length > 0 ? (
                    voiceChannels.map((channel) => {
                      ...
                      const participantsInChannel = isSelected && currentUserProfile
                        ? [{ userId: currentUserId, displayName: currentUserProfile.displayName || currentUserProfile.username, avatarUrl: currentUserProfile.avatarUrl }]
                        : [];
                      return (
                        <div key={channel._id} className={styles.voiceChannelWrap}>
                          ...
                          {isSelected && participantsInChannel.length > 0 && (
                            <div className={styles.voiceChannelParticipants}>
                              {participantsInChannel.map((p) => (
                                <div key={p.userId} className={styles.voiceChannelParticipant}>
                                  <div
                                    className={styles.voiceChannelParticipantAvatar}
                                    style={{
                                      backgroundImage: isValidAvatarUrl(p.avatarUrl) ? `url(${p.avatarUrl})` : undefined,
                                      backgroundColor: !isValidAvatarUrl(p.avatarUrl) ? "var(--color-primary)" : undefined,
                                      backgroundSize: "cover",
                                      backgroundPosition: "center",
                                    }}
                                  >
                                    {!isValidAvatarUrl(p.avatarUrl) && (
                                      <span>{(p.displayName || "?").charAt(0).toUpperCase()}</span>
                                    )}
                                  </div>
                                  <span className={styles.voiceChannelParticipantName}>{p.displayName}</span>
                                </div>
                              ))}
                            </div>
                          )}
