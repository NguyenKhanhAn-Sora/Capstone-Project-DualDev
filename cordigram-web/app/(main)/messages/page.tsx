...
export default function MessagesPage() {
  ...
  return (
    ...
    <div
      ...
      data-mobile-chat-open={Boolean(selectedDirectMessageFriend) || Boolean(selectedChannel) ? "true" : "false"}
    >
      {passkeyRequired ? (
                ...
                <div className={styles.chatHeader}>
                  <div className={styles.channelHeaderStart}>
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (selectedDirectMessageFriend) setSelectedDirectMessageFriend(null);
                  ...
                  <div className={styles.chatHeaderActions}>
                    ...
                    {selectedDirectMessageFriend && (
                      <>
                        ...
                        <button
                          type="button"
                          className={styles.chatActionMobileHidden}
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    {showPlusMenu && (
                      <div className={styles.plusMenuPopup}>
                        ...
                        {/* GIF — mobile only (hidden on desktop via CSS) */}
                        <button
                          className={`${styles.plusMenuItem} ${styles.plusMenuItemMobile}`}
                          onClick={() => {
                            setShowPlusMenu(false);
                            setMediaPickerTab("gif");
                            setShowGiphyPicker(true);
                        ...
                        >
                          ...
                          <span>{t("chat.composer.sendGif")}</span>
                        </button>
                        {/* Sticker — mobile only (hidden on desktop via CSS) */}
                        <button
                          className={`${styles.plusMenuItem} ${styles.plusMenuItemMobile}`}
                          onClick={() => {
                            setShowPlusMenu(false);
                        ...
                        >
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                            ...
                            <path d="M9 14.5c0.5 1 1.5 1.5 3 1.5s2.5-0.5 3-1.5" strokeLinecap="round" />
                          </svg>
                          <span>{t("chat.composer.sendSticker")}</span>
                        </button>
                      </div>
                    )}
                  </div>
                  ...
                  {!isRecordingVoice && !isUploadingVoice && (
                    <>
                      ...
                      <div className={styles.mediaButtons}>
                        ...
                        {/* GIF Button — hidden on mobile (moved to plus menu) */}
                        <button
                          type="button"
                          className={`${styles.mediaButton} ${styles.mediaButtonGif}`}
                        ...
                        {/* Sticker Button — hidden on mobile (moved to plus menu) */}
                        <button
                          type="button"
                          className={`${styles.mediaButton} ${styles.mediaButtonSticker}`}
            ...
            dmProfileSidebarOpen ? (
              <div className={styles.activeNowSidebar}>
                <div className={styles.activeNowHeader}>
                  {/* Back button — mobile only (chevron left) */}
                  <button
                    type="button"
                    className={`${styles.activeNowCloseBtn} ${styles.mobileOnly}`}
                  ...
                  <button
                    type="button"
                    className={`${styles.activeNowCloseBtn} ${styles.desktopOnly}`}
                    onClick={() => setDmProfileSidebarOpen(false)}
                    title={t("chat.profile.close")}
                    aria-label={t("chat.profile.close")}
                  >
              ...
              <button
                type="button"
                className={`${styles.dmProfileSidebarToggle} ${styles.chatActionMobileHidden}`}
      ...
      {dmContextMenu && (
        <DmConversationContextMenu
          ...
          onPhoneCall={!isSamePeerCallBlocked ? () => handleStartCall(false) : undefined}
          onVideoCall={!isSamePeerCallBlocked ? () => handleStartCall(true) : undefined}
          onSearch={() => {
            setMessageSearchDmConversationOnly(true);
            setShowMessageSearch(true);
          }}
          onViewProfile={() => setDmProfileSidebarOpen(true)}
          onConversationDetails={() => setDetailsPanelOpen((v) => !v)}
        />
      )}
