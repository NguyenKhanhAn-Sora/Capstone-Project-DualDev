...
import { pinPost, unpinPost, bulkDeletePosts } from "@/lib/api";
...
export default function ProfilePostsPage() {
  ...
  const isOwner = Boolean(
    viewerId && profile.userId && viewerId === profile.userId,
  );
  ...
  const handlePin = useCallback(async () => {
    ...
    if (toPin.length > 0 && currentPinnedNotSelected + toPin.length > MAX_PINS) {
      setPinError(
        t("profilePage.manage.pinLimitError", {
          max: String(MAX_PINS),
    ...
    try {
      ...
      await Promise.all(alreadyPinned.map((id) => unpinPost({ token, postId: id })));
      await Promise.all(toPin.map((id) => pinPost({ token, postId: id })));
  ...
  const handleDelete = useCallback(async () => {
    ...
    setShowDeleteConfirm(false);
    try {
      await bulkDeletePosts({ token, ids: Array.from(selectedIds) });
      setLocalItems((prev) =>
  ...
  return (
    <>
      ...
      {suppressGrid ? null : showEmpty && !manageMode ? (
        <div className={styles.errorBox}>{t("profilePage.manage.noPostsYet")}</div>
      ) : (
        <PostGrid
          items={displayItems}
          ...
          onSelect={(item) => {
            ...
            if (profile.id) {
              saveProfilePostNav(profile.id, displayItems);
            }

            const query = new URLSearchParams();
            query.set("fromProfile", "1");
            if (profile.id) query.set("profileId", profile.id);
            ...
            <div className={styles.modalHeader}>
              <div>
                <h2 className={styles.modalTitle}>
                  {t("profilePage.manage.deletePostTitle")}
                </h2>
                <p className={styles.modalBody}>
                  {t("profilePage.manage.deletePostBody", {
                    count: String(selectedIds.size),
                  })}
                </p>
              </div>
