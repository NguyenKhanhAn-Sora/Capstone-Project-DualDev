...
export default function InviteServerPage() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.replace(`/login?redirect=${encodeURIComponent(`/invite/server/${serverId}`)}`);
      return;
    }
    Promise.all([
  ...
  const handleAccept = async () => {
    if (!hasToken()) {
      router.push(`/login?redirect=${encodeURIComponent(`/invite/server/${serverId}`)}`);
      return;
    }
    setJoining(true);
    setError(null);
    try {
      // Apply-to-join: chỉ popup câu hỏi (không hiển thị quy định trong bước này).
  ...
  return (
    <div className="min-h-screen bg-[#313338] flex items-center justify-center p-4">
      <ApplyToJoinQuestionsModal
        open={applyModalOpen && !!applyForm && !!server}
        onClose={() => {
          if (joining) return;
          setApplyModalOpen(false);
          setError(null);
        ...
        submitting={joining}
        error={applyModalOpen ? error : null}
