...
export default function InviteServerChannelPage() {
  const params = useParams();
  const router = useRouter();
  const serverId = params.serverId as string;
  const channelId = params.channelId as string;
  const [server, setServer] = useState<serversApi.Server | null>(null);
  ...
  useEffect(() => {
    ...
    if (!token) {
      router.replace(`/login?redirect=${encodeURIComponent(`/invite/server/${serverId}/${channelId}`)}`);
      return;
    }
    Promise.all([
      serversApi.getServer(serverId).catch((e) => {
        throw e;
      }),
      serversApi.getMyServers(),
    ])
      .then(([s, myServers]) => {
        setServer(s);
        const channel = s.channels?.find((c: { _id: string }) => c._id === channelId);
        setChannelName(channel?.name ?? "Kênh thoại");
        const member = myServers.some((m) => m._id === serverId);
        setIsMember(member);
      })
      .catch((e) => setError(e instanceof Error ? e.message : "Không tải được máy chủ"))
      .finally(() => setLoading(false));
  }, [serverId, channelId, router]);

  useEffect(() => {
    if (server && !isMember && hasToken()) {
      const token = getToken();
      if (!token) return;
      fetchCurrentProfile({ token })
        .then((p) => setUserDisplayName(p.displayName || p.username || "Người dùng"))
  ...
  }, [server, isMember, serverId, channelId, router]);

  const handleAccept = async () => {
    if (!hasToken()) {
      router.push(`/login?redirect=${encodeURIComponent(`/invite/server/${serverId}/${channelId}`)}`);
      return;
    }
    setJoining(true);
    setError(null);
      ...
      <div className="w-full max-w-[420px] rounded-xl bg-[#2b2d31] shadow-xl overflow-hidden">
        ...
        <div className="pt-8 pb-4 px-6 text-center">
          ...
          <p className="text-[#b5bac1] text-sm mb-4">
            Sau khi tham gia, bạn sẽ được đưa đến kênh thoại &quot;{channelName}&quot;
          </p>
          <div className="flex justify-center mb-4">
            <div
