...
interface Props {
  ...
  autoPlay?: boolean;
  /** Cloudinary URL of a WebVTT caption file generated by the backend */
  captionUrl?: string | null;
  /** BCP-47 language code of the caption track (e.g. 'vi', 'en') */
...
const CustomVideoPlayer = forwardRef<HTMLVideoElement, Props>(
  function CustomVideoPlayer(
    ...
    // Sync caption track mode with showCaptions state
    useEffect(() => {
      const v = internalRef.current;
      if (!v || !captionUrl) return;
      const syncMode = () => {
        const track = v.textTracks?.[0];
        if (track) track.mode = showCaptions ? "showing" : "hidden";
      };
      // Track may not be ready yet if video just mounted; retry after a tick
      syncMode();
      const timer = setTimeout(syncMode, 100);
      return () => clearTimeout(timer);
    }, [showCaptions, captionUrl, currentSrc]);
        ...
        >
          ...
          <div className={styles.bottomBar}>
            ...
            <div className={styles.rightGroup}>
              ...
              {/* Captions (CC) — only when a caption track exists */}
              {captionUrl && (
                <button
                  type="button"
                  className={`${styles.iconBtn} ${showCaptions ? styles.ccActive : ""}`}
...
function CcIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
      ...
      <text x="5" y="16" fontSize="8" fontWeight="700" fontFamily="Arial,sans-serif" fill="currentColor" stroke="none">CC</text>
    </svg>
  );
}
