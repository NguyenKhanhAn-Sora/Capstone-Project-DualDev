...
export class ReportPostService {
  ...
  private async evaluateAndAutoHidePost(params: {
    postId: string;
    fallbackModeratorId: string;
  }): Promise<void> {
    const { postId, fallbackModeratorId } = params;
    const post = await this.postModel
      .findById(postId)
      .select('moderationState autoHiddenPendingReview')
      .lean();
    ...
    const meetsStandardRule =
      ...
      uniqueReporters >= 4 &&
      severeTopCategory &&
      (reportsLast1h >= 2 || reportsLast24h >= 5);
    const meetsCriticalRule =
      (score >= 11 && uniqueReporters >= 6) || severeReasonCount >= 2;
    ...
    const updateResult = await this.postModel
      .updateOne(
        { _id: postId, autoHiddenPendingReview: { $ne: true } },
        {
          $set: {
  ...
  async create(
    ...
    if (existing) {
      ...
      await this.evaluateAndAutoHidePost({
        postId,
