/**
 * Discord-style message search query parser.
 * Supports: from:, in:, has: (image | file)
 * Remaining tokens are free-text search terms.
 */

export type MessageSearchHasType = 'image' | 'file';

export interface ParsedMessageSearchFilters {
  /** User id (ObjectId) or username hint */
  from?: string;
  /** Channel id (ObjectId) or channel name */
  in?: string;
  /** Restrict to messages with images / attachments */
...
/**
 * Parses a raw search string into free text and structured filters.
 */
...
export function parseMessageSearchQueryForDm(
  input: string,
): ParsedMessageSearch {
